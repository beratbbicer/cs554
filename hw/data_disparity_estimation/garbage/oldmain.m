...
for j = 1:20
    ...
    [x,~] = size(matching_points2);
    matching_pts_homography = zeros(x,2);
    for i=1:x
        [xp, yp] = get_correspondance(h, matching_points1(i,1), matching_points1(i,2));
        matching_pts_homography(i,:) = [ceil(xp - xmin + 1), ceil(yp - ymin + 1)];
    end
    
    figure;
    [fLMedS,inliers] = estimateFundamentalMatrix(matching_pts_homography, matching_points2,'NumTrials',4000);
    subplot(121);
    imshow(rectified_img); 
    title('Inliers and Epipolar Lines in First Image'); hold on;
    plot(matching_pts_homography(inliers,1), matching_pts_homography(inliers,2),'go');
    ...
    epiLines = epipolarLine(fLMedS,matching_pts_homography(inliers,:));
